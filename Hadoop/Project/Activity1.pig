...
inputDialogues4 = LOAD '/user/devika/inputs/episodeIV_dialogues.txt' using PigStorage('\t') AS (name:chararray,line:chararray);
inputDialogues5 = LOAD '/user/devika/inputs/episodeV_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
inputDialogues6 = LOAD '/user/devika/inputs/episodeVI_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
...
rmf hdfs:///user/devika/Outputs;

-- Store the result in HDFS
STORE namesOrdered INTO 'hdfs:///user/devika/Outputs' using PigStorage('\t');